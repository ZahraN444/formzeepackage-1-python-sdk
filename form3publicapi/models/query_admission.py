# -*- coding: utf-8 -*-

"""
form3publicapi

This file was automatically generated by APIMATIC v3.0 (
 https://www.apimatic.io ).
"""
from form3publicapi.api_helper import APIHelper
from form3publicapi.models.query_admission_attributes import QueryAdmissionAttributes
from form3publicapi.models.query_admission_relationships import QueryAdmissionRelationships


class QueryAdmission(object):

    """Implementation of the 'QueryAdmission' model.

    TODO: type model description here.

    Attributes:
        attributes (QueryAdmissionAttributes): TODO: type description here.
        created_on (datetime): TODO: type description here.
        id (uuid|str): TODO: type description here.
        modified_on (datetime): TODO: type description here.
        organisation_id (uuid|str): TODO: type description here.
        relationships (QueryAdmissionRelationships): TODO: type description
            here.
        mtype (str): TODO: type description here.
        version (int): TODO: type description here.

    """

    # Create a mapping from Model property names to API property names
    _names = {
        "attributes": 'attributes',
        "id": 'id',
        "organisation_id": 'organisation_id',
        "mtype": 'type',
        "version": 'version',
        "created_on": 'created_on',
        "modified_on": 'modified_on',
        "relationships": 'relationships'
    }

    _optionals = [
        'created_on',
        'modified_on',
        'relationships',
    ]

    def __init__(self,
                 attributes=None,
                 id=None,
                 organisation_id=None,
                 version=None,
                 created_on=APIHelper.SKIP,
                 modified_on=APIHelper.SKIP,
                 relationships=APIHelper.SKIP):
        """Constructor for the QueryAdmission class"""

        # Initialize members of the class
        self.attributes = attributes 
        if created_on is not APIHelper.SKIP:
            self.created_on = APIHelper.apply_datetime_converter(created_on, APIHelper.RFC3339DateTime) if created_on else None 
        self.id = id 
        if modified_on is not APIHelper.SKIP:
            self.modified_on = APIHelper.apply_datetime_converter(modified_on, APIHelper.RFC3339DateTime) if modified_on else None 
        self.organisation_id = organisation_id 
        if relationships is not APIHelper.SKIP:
            self.relationships = relationships 
        self.mtype = 'query_admissions' 
        self.version = version 

    @classmethod
    def from_dictionary(cls,
                        dictionary):
        """Creates an instance of this model from a dictionary

        Args:
            dictionary (dictionary): A dictionary representation of the object
            as obtained from the deserialization of the server's response. The
            keys MUST match property names in the API description.

        Returns:
            object: An instance of this structure class.

        """

        if dictionary is None:
            return None

        # Extract variables from the dictionary
        attributes = QueryAdmissionAttributes.from_dictionary(dictionary.get('attributes')) if dictionary.get('attributes') else None
        id = dictionary.get("id") if dictionary.get("id") else None
        organisation_id = dictionary.get("organisation_id") if dictionary.get("organisation_id") else None
        version = dictionary.get("version") if dictionary.get("version") else None
        created_on = APIHelper.RFC3339DateTime.from_value(dictionary.get("created_on")).datetime if dictionary.get("created_on") else APIHelper.SKIP
        modified_on = APIHelper.RFC3339DateTime.from_value(dictionary.get("modified_on")).datetime if dictionary.get("modified_on") else APIHelper.SKIP
        relationships = QueryAdmissionRelationships.from_dictionary(dictionary.get('relationships')) if 'relationships' in dictionary.keys() else APIHelper.SKIP
        # Return an object of this model
        return cls(attributes,
                   id,
                   organisation_id,
                   version,
                   created_on,
                   modified_on,
                   relationships)
